...
export async function POST(request: Request) {
  try {
    ...
    const { fullName, email, telephone, company, subject, message, honeypot } = body;
    ...
    const rateLimit = isRateLimited(`contact:${ip}`);
    ...
    if (!fullName || !email || !telephone || !subject || !message) {
